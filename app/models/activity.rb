class Activity < ActiveRecord::Base
  ...
  def self.create_activity(user, subject, action)
    unless subject.is_a?(Product) && action == :viewed # Tasks don't have landing pages, so technically they can't be "viewed".
      create(
        :user    => user,
        :subject => subject,
        :action  => action.to_s,
        :info    => subject.respond_to?(:full_name) ? subject.full_name : (subject.respond_to?(:name) ? subject.name : subject.number)
      )
    end
  end
